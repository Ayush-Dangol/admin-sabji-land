...
function OrderView(props) {
    ...
    return (
        <div className="Vpop-container" style={props.vPop ? { display: "block" } : { display: "none" }}>
            <div className="view-content">
                <div className="logo"><img src="./Images/logo.jpg" alt="logo" /></div>
                <div className="close-button"><button onClick={props.toggleV}><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none"><path fill="#777" d="M11 .5C5.15.5.5 5.15.5 11S5.15 21.5 11 21.5 21.5 16.85 21.5 11 16.85.5 11 .5Zm4.05 15.75L11 12.2l-4.05 4.05-1.2-1.2L9.8 11 5.75 6.95l1.2-1.2L11 9.8l4.05-4.05 1.2 1.2L12.2 11l4.05 4.05-1.2 1.2Z" /></svg></button></div>
                <div className="heading">Invoice</div>

                {filterView.map((dataV) => {

                    const { id, name, email, date, orderS, orderA, contact, address, delivered, pDate, pStatus, pMethod, oProducts, dCharge } = dataV
                    return (
                        <>
                            ...
                            <div className="invoice-table">
                                <table>
                                    <thead>
                                        <tr>
                                            <th style={{ textAlign: "left", paddingLeft: "20px" }}>Product Name</th>
                                            <th>Rate</th>
                                            <th>Qty</th>
                                    ...
                                    <tbody>
                                        {oProducts.map((oP) => {
                                            ...
                                            return (
                                                <>
                                                    <tr>
                                                        <td style={{ textAlign: "left", paddingLeft: "20px" }}>{pName}</td>
                                                        <td>{pRate}</td>
                                                        <td>{pQty}</td>
                                                        <td>{amount}</td>
                                                    </tr>
